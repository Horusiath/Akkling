...
[<Fact>]
let ``Actor computation try-with should not overflow the stack`` () =
    testDefault
    <| fun tck ->
...
[<Fact>]
let ``Actor computation try-with handles exception`` () =
    testDefault
    <| fun tck ->
        let ref =
            spawn
...
[<Fact>]
let ``Actor computation try-with after receive handles exception`` () =
    testDefault
    <| fun tck ->
        let ref =
            spawn
...
[<Fact>]
let ``Actor computation try-finally should not overflow the stack`` () =
    testDefault
    <| fun tck ->
        let complete = TaskCompletionSource<_>()
...
[<Fact>]
let ``Actor computation try-finally executes finally`` () =
    testDefault
    <| fun tck ->
        let ref =
            spawn
...
[<Fact>]
let ``Actor computation try-finally after receive executes finally`` () =
    testDefault
    <| fun tck ->
        let ref =
            spawn
...
[<Fact>]
let ``Actor computation use should not overflow the stack`` () =
    testDefault
    <| fun tck ->
        let complete = TaskCompletionSource<_>()
...
[<Fact>]
let ``Actor computation use disposes objects`` () =
    testDefault
    <| fun tck ->
        let ref =
            spawn
...
[<Fact>]
let ``Actor computation use before receive disposes objects`` () =
    testDefault
    <| fun tck ->
        let ref =
            spawn
...
[<Fact>]
let ``Actor try-with should generate tail call`` () =
    testDefault
    <| fun tck ->
        let complete = TaskCompletionSource<_>()
