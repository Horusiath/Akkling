        ...
        actor {
            ...
            | :? SenderMsg as snd ->
                ...
                | Req payload ->
                    ...
                    ctx.Sender() <! ReqAck
                | ActionAck id -> alod.Confirm id |> ignore
...
[<Fact>]
let ``at-least-once delivery semantics should redeliver messages`` () =
    test config
    <| fun tck ->
        ...
        let destinations =
            Map.ofList [ 'a', (spawn tck "unreliable" <| props (unreliable 3 dest)).Path.ToString() ]
...
[<Fact>]
let ``PersistenceLifecycleEvent should be fired`` () =
    testDefault
    <| fun tck ->
        let pref =
            spawn tck "p-1"
...
[<Fact>]
let ``Effects.andThen should be called after event was persisted`` () =
    testDefault
    <| fun tck ->
