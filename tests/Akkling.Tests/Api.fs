...
type ItemAvailabilityReply = AvailabilityReply of Handle: string * Availability: Availability
...
let ``can serialize discriminated unions`` () =
    ...
    let des =
        serializer.FromBinary(bytes, typeof<TraceableMessage<Availability>>) :?> TraceableMessage<Availability>
...
[<Fact>]
let ``Typed actor refs are serializable/deserializable in both directions`` () : unit =
    testDefault
    <| fun tck ->
...
[<Fact>]
let ``Typed props are serializable/deserializable in both directions`` () : unit =
    testDefault
    <| fun tck ->
        let p =
            { (props Behaviors.echo) with
...
type InnerUnion = Inner of int * string
...
let ``can serialize and deserialize discriminated unions over remote nodes`` () =
    ...
    let aref =
        spawn
            client
            "a-1"
            { (props (actorOf2 testBehavior)) with
                Deploy = Some(Deploy(RemoteScope(Address.Parse "akka.tcp://server-system@localhost:9911"))) }
    ...
    let aref2 =
        spawn
            ...
            "a-1"
            { (props (actorOf2 testBehavior)) with
                Deploy = Some(Deploy(RemoteScope(Address.Parse "akka.tcp://server-system@localhost:9912"))) }
...
[<Fact>]
let ``monitor works on typed refs`` () =
    testDefault
    <| fun tck ->
...
[<Fact>]
let ``demonitor works on typed refs`` () =
    testDefault
    <| fun tck ->
        let ref = spawn tck "actor" (props Behaviors.echo)
        monitor tck ref
...
[<Fact(Skip = "FIXME")>]
let ``custom supervisor strategy works with routers`` () =
    testDefault
    <| fun tck ->
