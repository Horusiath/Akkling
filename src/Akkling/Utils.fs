...
module Watchers =
    ...
    /// </summary>
    let inline monitorWith (reply: 'Reply) (watcher: #ICanWatch) (subject: IActorRef<'Message>) : IActorRef =
        watcher.WatchWith(untyped subject, box reply)

    /// <summary>
    /// Orders a <paramref name="watcher"/> to stop monitoring an actor referred by provided <paramref name="subject"/>.
    /// </summary>
    let inline demonitor (watcher: #ICanWatch) (subject: IActorRef<'Message>) : IActorRef =
        watcher.Unwatch(untyped subject)
