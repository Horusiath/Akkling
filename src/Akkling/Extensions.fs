...
let (|LifecycleEvent|_|) (message: obj) : LifecycleEvent option = message |> asLifecycleEvent
...
let (|PostRestart|_|) (message: obj) : exn option =
    ...
    |> asLifecycleEvent
    |> Option.bind (function
        | PostRestart c -> Some c
        | _ -> None)

let inline (<@>) (x: Effect<'Message>) (y: Effect<'Message>) : Effect<'Message> =
    CombinedEffect(x, y) :> Effect<'Message>
