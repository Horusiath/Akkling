...
module HoconExtensions =
    ...
    let (?) (conf: Config) (prop: string) : 'a =
        ...
        if t = typeof<Config> then
            box (conf.GetConfig(path)) :?> 'a
        elif t = typeof<string> then
            box (conf.GetString(path)) :?> 'a
        elif t = typeof<int> then
            box (conf.GetInt(path)) :?> 'a
        elif t = typeof<int64> then
            box (conf.GetLong(path)) :?> 'a
        elif t = typeof<bool> then
            box (conf.GetBoolean(path)) :?> 'a
        elif t = typeof<TimeSpan> then
            box (conf.GetTimeSpan(path)) :?> 'a
        elif t = typeof<float32> then
            box (conf.GetFloat(path)) :?> 'a
        elif t = typeof<float> then
            box (conf.GetDouble(path)) :?> 'a
        elif t = typeof<decimal> then
            box (conf.GetDecimal(path)) :?> 'a
        elif t = typeof<string seq> then
            box (conf.GetStringList(path)) :?> 'a
        elif t = typeof<int seq> then
            box (conf.GetIntList(path)) :?> 'a
        elif t = typeof<bool seq> then
            box (conf.GetBooleanList(path)) :?> 'a
        elif t = typeof<byte seq> then
            box (conf.GetByteList(path)) :?> 'a
        elif t = typeof<int64 seq> then
            box (conf.GetLongList(path)) :?> 'a
        elif t = typeof<float seq> then
            box (conf.GetFloatList(path)) :?> 'a
        elif t = typeof<double seq> then
            box (conf.GetDoubleList(path)) :?> 'a
        elif t = typeof<decimal seq> then
            box (conf.GetDecimalList(path)) :?> 'a
        elif typeof<Enum>.GetTypeInfo().IsAssignableFrom t then
            Enum.Parse(t, conf.GetString path, true) :?> 'a
        else
            failwithf "Result type of %O is not supported" t
...
module Configuration =
    ...
    let defaultConfig () =
        extendedConfig.WithFallback(Akka.Configuration.ConfigurationFactory.Default())
