...
//     Copyright (C) 2016-2024 Bartosz Sypytkowski, Vagif Abilov and contributors <https://github.com/Horusiath/Akkling>
// </copyright>
//-----------------------------------------------------------------------
...
module GSet =
    ...
    let ofList<'t> (list: 't list) =
        list |> List.fold (fun gset e -> add e gset) empty

    let ofArray<'t> (array: 't[]) =
        array |> Array.fold (fun gset e -> add e gset) empty

    let ofSeq<'t> (seq: 't seq) =
        seq |> Seq.fold (fun gset e -> add e gset) empty
...
module ORSet =
    ...
    let ofList<'t> (node: Cluster) (list: 't list) =
        list
        |> List.map (fun e -> KeyValuePair(node.SelfUniqueAddress, e))
        |> ORSet.Create

    let ofArray<'t> (node: Cluster) (array: 't[]) =
        array
        |> Array.map (fun e -> KeyValuePair(node.SelfUniqueAddress, e))
        |> ORSet.Create

    let ofSeq<'t> (node: Cluster) (seq: 't seq) =
        seq
        |> Seq.map (fun e -> KeyValuePair(node.SelfUniqueAddress, e))
        |> ORSet.Create
...
module ORMap =
    ...
    let empty<'k, 'v when 'v :> IReplicatedData<'v>> : ORMap<'k, 'v> =
        ORMap<'k, 'v>.Empty
    ...
    let ofList (node: Cluster) (list) : ORMap<_, _> =
        list
        |> List.map (fun (k, v) -> struct (node.SelfUniqueAddress, k, v))
        |> ORDictionary.Create

    let ofArray (node: Cluster) (array) : ORMap<_, _> =
        array
        |> Array.map (fun (k, v) -> struct (node.SelfUniqueAddress, k, v))
        |> ORDictionary.Create

    let ofMap (node: Cluster) (map) : ORMap<_, _> =
        map
        |> Map.toSeq
        |> Seq.map (fun (k, v) -> struct (node.SelfUniqueAddress, k, v))
        |> ORDictionary.Create

    let ofSeq (node: Cluster) (seq) : ORMap<_, _> =
        seq
        |> Seq.map (fun (k, v) -> struct (node.SelfUniqueAddress, k, v))
        |> ORDictionary.Create

    let toList (x: ORMap<_, _>) =
        x.Entries |> Seq.map (fun entry -> (entry.Key, entry.Value)) |> List.ofSeq

    let toArray (x: ORMap<_, _>) =
        x.Entries |> Seq.map (fun entry -> (entry.Key, entry.Value)) |> Array.ofSeq

    let toSeq (x: ORMap<_, _>) =
        x.Entries |> Seq.map (fun entry -> (entry.Key, entry.Value))

    let toMap (x: ORMap<_, _>) =
        x.Entries |> Seq.map (fun entry -> (entry.Key, entry.Value)) |> Map.ofSeq
...
module ORMultiMap =
    ...
    let empty<'k, 'v when 'v :> IReplicatedData> : ORMultiMap<'k, 'v> =
        ORMultiMap<'k, 'v>.Empty

    let inline merge (x: ORMultiMap<_, _>) (y: ORMultiMap<_, _>) = x.Merge(y)
    let inline value (x: ORMultiMap<_, _>) = x.Entries

    let inline add (node: Cluster) k (items: 'v seq) (x: ORMultiMap<_, _>) =
        x.SetItems(node, k, System.Collections.Immutable.ImmutableHashSet.Create<'v>(items |> Seq.toArray))
    ...
    let toSeq (x: ORMultiMap<_, _>) =
        x.Entries |> Seq.map (fun entry -> (entry.Key, entry.Value |> Set.ofSeq))
...
module LWWReg =
    ...
    let inline create<'t> (node: Cluster) (value: 't) : LWWReg<'t> =
        LWWRegister(node.SelfUniqueAddress, value)

    let inline createWithTimestamp<'t> (node: Cluster) (timestamp: int64) (value: 't) : LWWReg<'t> =
        LWWRegister(node.SelfUniqueAddress, value, timestamp)

    let inline createWithClock<'t> (node: Cluster) (clock: int64 -> 't -> int64) (value: 't) : LWWReg<'t> =
        LWWRegister(node.SelfUniqueAddress, value, Clock<'t>(clock))
...
module LWWMap =
    ...
    let ofList (node: Cluster) (list) : LWWMap<_, _> =
        list
        |> List.map (fun (k, v) -> struct (node.SelfUniqueAddress, k, v))
        |> LWWDictionary.Create

    let ofArray (node: Cluster) (array) : LWWMap<_, _> =
        array
        |> Array.map (fun (k, v) -> struct (node.SelfUniqueAddress, k, v))
        |> LWWDictionary.Create

    let ofMap (node: Cluster) (map) : LWWMap<_, _> =
        map
        |> Map.toSeq
        |> Seq.map (fun (k, v) -> struct (node.SelfUniqueAddress, k, v))
        |> LWWDictionary.Create

    let ofSeq (node: Cluster) (seq) : LWWMap<_, _> =
        seq
        |> Seq.map (fun (k, v) -> struct (node.SelfUniqueAddress, k, v))
        |> LWWDictionary.Create

    let toList (x: LWWMap<_, _>) =
        x.Entries |> Seq.map (fun entry -> (entry.Key, entry.Value)) |> List.ofSeq

    let toArray (x: LWWMap<_, _>) =
        x.Entries |> Seq.map (fun entry -> (entry.Key, entry.Value)) |> Array.ofSeq

    let toSeq (x: LWWMap<_, _>) =
        x.Entries |> Seq.map (fun entry -> (entry.Key, entry.Value))

    let toMap (x: LWWMap<_, _>) =
        x.Entries |> Seq.map (fun entry -> (entry.Key, entry.Value)) |> Map.ofSeq
...
module PNCounterMap =
    ...
    let toList (x: PNCounterMap<_>) =
        x.Entries |> Seq.map (fun entry -> (entry.Key, entry.Value)) |> List.ofSeq

    let toArray (x: PNCounterMap<_>) =
        x.Entries |> Seq.map (fun entry -> (entry.Key, entry.Value)) |> Array.ofSeq

    let toSeq (x: PNCounterMap<_>) =
        x.Entries |> Seq.map (fun entry -> (entry.Key, entry.Value))

    let toMap (x: PNCounterMap<_>) =
        x.Entries |> Seq.map (fun entry -> (entry.Key, entry.Value)) |> Map.ofSeq
