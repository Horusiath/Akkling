...
module ORMap = 
    ...
    let ofList (node: Cluster) (list) : ORMap<_,_> = list |> List.map (fun (k, v) -> struct (node.SelfUniqueAddress, k, v)) |> ORDictionary.Create
    let ofArray (node: Cluster) (array) : ORMap<_,_> = array |> Array.map (fun (k, v) -> struct (node.SelfUniqueAddress, k, v)) |> ORDictionary.Create
    let ofMap (node: Cluster) (map) : ORMap<_,_> = map |> Map.toSeq |> Seq.map (fun (k, v) -> struct (node.SelfUniqueAddress, k, v)) |> ORDictionary.Create
    let ofSeq (node: Cluster) (seq) : ORMap<_,_> = seq |> Seq.map (fun (k, v) -> struct (node.SelfUniqueAddress, k, v)) |> ORDictionary.Create
...
module LWWMap = 
    ...
    let ofList (node: Cluster) (list) : LWWMap<_,_> = list |> List.map (fun (k, v) -> struct (node.SelfUniqueAddress, k, v)) |> LWWDictionary.Create
    let ofArray (node: Cluster) (array) : LWWMap<_,_> = array |> Array.map (fun (k, v) -> struct (node.SelfUniqueAddress, k, v)) |> LWWDictionary.Create
    let ofMap (node: Cluster) (map) : LWWMap<_,_> = map |> Map.toSeq |> Seq.map (fun (k, v) -> struct (node.SelfUniqueAddress, k, v)) |> LWWDictionary.Create
    let ofSeq (node: Cluster) (seq) : LWWMap<_,_> = seq |> Seq.map (fun (k, v) -> struct (node.SelfUniqueAddress, k, v)) |> LWWDictionary.Create
