...
module FlowWithContext =
    ...
    let inline filter (fn: 't -> bool) (flow: FlowWithContext<_,_,_,_,_>) =
        flow.Where(Func<_, _>(fn))
...
module SourceWithContext =
    ...
    let inline filter (fn: 't -> bool) (source: SourceWithContext<_,_,_>) =
        source.Where(Func<_, _>(fn))
