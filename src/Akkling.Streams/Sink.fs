...
module Sink =
    ...
    let inline ofSubscriber (sub: #ISubscriber<_>) : Sink<_, unit> =
        Sink.FromSubscriber(sub).MapMaterializedValue(Func<_, _>(ignore))

    /// A sink that materializes into an Async computation of the first value received.
    let inline head<'t> : Sink<'t, Async<'t>> =
        Sink.First<'t>().MapMaterializedValue(Func<_, _>(Async.AwaitTask))

    /// A sink that materializes into an Async computation of the first value received.
    let inline headOption<'t> : Sink<'t, Async<'t>> =
        Sink.FirstOrDefault<'t>().MapMaterializedValue(Func<_, _>(Async.AwaitTask))

    /// A sink that materializes into an Async computation of the last value received.
    let inline tail<'t> : Sink<'t, Async<'t>> =
        Sink.Last<'t>().MapMaterializedValue(Func<_, _>(Async.AwaitTask))

    /// A sink that materializes into an Async computation of the last value received.
    let inline tailOption<'t> : Sink<'t, Async<'t>> =
        Sink.LastOrDefault<'t>().MapMaterializedValue(Func<_, _>(Async.AwaitTask))
    ...
    let inline fanoutPublisher<'t> : Sink<'t, IPublisher<'t>> =
        Sink.FanoutPublisher<'t>()

    /// A sink that will consume the stream and discard the elements.
    let inline ignore<'t> : Sink<'t, Async<Akka.Done>> =
        Sink.Ignore<'t>().MapMaterializedValue(Func<_, _>(Async.AwaitTask))
    ...
    /// the stream.
    let inline forEach (fn: 't -> unit) : Sink<'t, Async<Akka.Done>> =
        Sink.ForEach(Action<_>(fn)).MapMaterializedValue(Func<_, _>(Async.AwaitTask))
    ...
    let inline onComplete (fn: exn option -> unit) : Sink<'t, unit> =
        Sink
            .OnComplete(Action(fun () -> fn (None)), Action<_>(fun e -> fn (Some e)))
    ...
    let inline toActorRefAck
        ...
        : Sink<'t, unit> =
        Sink
            .ActorRefWithAck(untyped ref, initMsg, ackMsg, completeMsg, Func<_, _>(failMsgFn >> box))
            .MapMaterializedValue(Func<_, _>(Microsoft.FSharp.Core.Operators.ignore))
    ...
    let inline ofProps (props: Props<'t>) : Sink<'t, IActorRef<'t>> =
        Sink.ActorSubscriber(props.ToProps()).MapMaterializedValue(Func<_, _>(typed))
    ...
    /// to read the values produced by the stream this Sink is attached to.
    /// This Sink is intended for inter-operation with legacy APIs since it is inherently blocking.
    let inline asStreamWithTimeout (timeout: TimeSpan) : Sink<ByteString, Stream> =
        StreamConverters.AsInputStream(Nullable timeout)

    /// Transform the materialized value of this Source, leaving all other properties as they were.
    let inline mapMatValue (fn: 'mat -> 'mat2) (sink: Sink<'t, 'mat>) =
        sink.MapMaterializedValue(Func<'mat, 'mat2> fn)
    ...
    let statefulPartition
        ...
        (initialState: 'state)
        (partitioner: PartitionHub.IConsumerInfo -> 'state -> 't -> 'state * int64)
        : Sink<'t, Source<'t, unit>> =
        let partitionFunc: Func<Func<PartitionHub.IConsumerInfo, 't, int64>> =
            Func<_>(fun () ->
