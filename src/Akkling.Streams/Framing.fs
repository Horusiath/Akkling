...
module Framing =
    ...
    let inline delimiter (allowTruncate: bool) (maxLength: int) (delim: ByteString) : Flow<ByteString, ByteString, unit> = 
        Akka.Streams.Dsl.Framing.Delimiter(delim, maxLength, allowTruncate).MapMaterializedValue(Func<_,_>(ignore))
    ...
    /// a truncated frame.
    let inline lengthFieldWithOffset (offset: int) (order: ByteOrder) (fieldLength: int) (maxLength: int) : Flow<ByteString, ByteString, unit> =
        Akka.Streams.Dsl.Framing.LengthField(fieldLength, maxLength, offset, order).MapMaterializedValue(Func<_,_>(ignore))
    ...
    let inline simpleEncoder (maxLength: int): Flow<ByteString, ByteString, unit> =
        Akka.Streams.Dsl.Framing.SimpleFramingProtocolEncoder(maxLength).MapMaterializedValue(Func<_,_>(ignore))
        
    let inline simpleDecoder (maxLength: int): Flow<ByteString, ByteString, unit> =
        Akka.Streams.Dsl.Framing.SimpleFramingProtocolDecoder(maxLength).MapMaterializedValue(Func<_,_>(ignore))
    ...
    let inline json (maxObjectLength: int): Flow<ByteString, ByteString, unit> =
        Akka.Streams.Dsl.JsonFraming.ObjectScanner(maxObjectLength).MapMaterializedValue(Func<_,_>(ignore))