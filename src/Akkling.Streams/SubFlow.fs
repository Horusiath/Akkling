...
module SubFlow =
    ...
    let recover (fn: exn -> 'out option) (subFlow: SubFlow<'out, 'mat, 'closed>) : SubFlow<'out option, 'mat, 'closed> =
        SubFlowOperations
            .Recover(subFlow, Func<exn, Akka.Util.Option<'out>>(fn >> toCsOption))
            .Select(Func<Akka.Util.Option<'out>, 'out option>(ofCsOption))
    ...
    let inline recoverWithRetries
        ...
        (subFlow)
        : SubFlow<'out, 'mat, 'closed> =
        SubFlowOperations.RecoverWithRetries(
            subFlow,
    ...
    let inline filter (pred: 'u -> bool) (subFlow) : SubFlow<'u, 'mat, 'closed> =
        SubFlowOperations.Where(subFlow, Predicate<_>(pred))
    ...
    /// of this step.
    let inline takeWhile (pred: 'u -> bool) (subFlow) : SubFlow<'u, 'mat, 'closed> =
        SubFlowOperations.TakeWhile(subFlow, Predicate<_>(pred))
    ...
    let inline skipWhile (pred: 'u -> bool) (subFlow) : SubFlow<'u, 'mat, 'closed> =
        SubFlowOperations.SkipWhile(subFlow, Predicate<_>(pred))
    ...
    /// Apply a sliding window over the stream and return the windows as groups of elements, with the last group
    /// possibly smaller than requested due to end-of-stream.
    let inline slidingBy (n: int) (step: int) (subFlow) : SubFlow<'u seq, 'mat, 'closed> =
        SubFlowOperations.Sliding(subFlow, n, step)
    ...
    let inline asyncScan
        (folder: 'state -> 'inp -> Async<'state>)
        (zero: 'state)
        (subFlow)
        : SubFlow<'state, 'mat, 'closed> =
    ...
    let inline delay (by: TimeSpan) (subFlow) : SubFlow<'u, 'mat, 'closed> = SubFlowOperations.Delay(subFlow, by)
    ...
    ///
    /// Delay precision is 10ms to avoid unnecessary timer scheduling cycles
    let inline delayWithStrategy
        (strategy: DelayOverflowStrategy)
        (by: TimeSpan)
        (subFlow)
        : SubFlow<'u, 'mat, 'closed> =
    ...
    let inline skip (n: int64) (subFlow) : SubFlow<'u, 'mat, 'closed> = SubFlowOperations.Skip(subFlow, n)
    ...
    /// or negative.
    let inline take (n: int64) (subFlow) : SubFlow<'u, 'mat, 'closed> = SubFlowOperations.Take(subFlow, n)
    ...
    let inline buffer (strategy: OverflowStrategy) (n: int) (subFlow) : SubFlow<'t, 'u, 'mat> =
        SubFlowOperations.Buffer(subFlow, n, strategy)
    ...
    let inline collectMerge
        (breadth: int)
        (flatten: 'u -> #IGraph<SourceShape<'w>, 'mat>)
        (subFlow)
        : SubFlow<'w, 'mat, 'closed> =
    ...
    let inline zipi (subFlow) : SubFlow<struct ('out * int64), 'mat, 'closed> = SubFlowOperations.ZipWithIndex(subFlow)
    ...
    let inline detach (subFlow) : SubFlow<'u, 'mat, 'closed> = SubFlowOperations.Detach(subFlow)
    ...
    let inline deduplicate
        (eq: 'out -> 'out -> bool)
        (subFlow: SubFlow<'out, 'mat, 'closed>)
        : SubFlow<'out, 'mat, 'closed> =
