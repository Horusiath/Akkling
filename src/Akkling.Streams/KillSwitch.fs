...
module KillSwitch =
    ...
    let inline ofCancellationTokenGraceful (c: CancellationToken) =
        KillSwitches.AsFlow(c, cancelGracefully = true)
    ...
    let inline ofCancellationTokenAbrupt (c: CancellationToken) =
        KillSwitches.AsFlow(c, cancelGracefully = false)
