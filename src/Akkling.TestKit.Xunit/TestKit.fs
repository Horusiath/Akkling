...
let expectMsg (tck: Tck) (msg: 't) : 't option =
    ...
    if reply <> Unchecked.defaultof<'t> then
        Some reply
    else
        None
...
let inline expectMsgAllOf (tck: Tck) (messages: 't seq) : unit =
    Array.ofSeq messages |> tck.ExpectMsgAllOf |> ignore

let inline expectMsgAnyOf (tck: Tck) (messages: 't seq) : unit =
    tck.ExpectMsgAnyOf(Array.ofSeq messages) |> ignore
...
let inline ignoreMessages (tck: Tck) (predicate: obj -> bool) : unit =
    tck.IgnoreMessages(Func<obj, bool>(predicate))

let inline receiveN (tck: Tck) (n: int) : obj seq =
    tck.ReceiveN(n) :> System.Collections.Generic.IEnumerable<obj>

let inline receiveOne (tck: Tck) : 't = tck.ReceiveOne() :?> 't

let inline receiveOneWithin (tck: Tck) (timeout: TimeSpan) : 't =
    tck.ReceiveOne(Nullable(timeout)) :?> 't
...
let inline expectEventWithin (timeout: TimeSpan) (n: int) (applier: IEventFilterApplier) (fn: unit -> 't) : 't =
    applier.Expect(n, timeout, Func<'t>(fn))
