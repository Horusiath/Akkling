...
type internal TypedMessageExtractorWithShardResolver<'Envelope, 'Message>(extractor: Extractor<_,'Message>, shardResolver :ShardResolver) =
    inherit TypedMessageExtractor<'Envelope, 'Message>(extractor)
    interface IMessageExtractor with
...
/// <param name="name"></param>
/// <param name="props"></param>
let rememberedEntityFactoryFor (system: ActorSystem) (shardResolver:ShardResolver) (name: string) (props: Props<'Message>) : EntityFac<'Message> =
    let clusterSharding = ClusterSharding.Get(system)
    let adjustedProps = adjustPersistentProps props
    let shardSettings = ClusterShardingSettings.Create(system).WithRememberEntities(true)
    let shardRegion =
        clusterSharding.Start(name, adjustedProps.ToProps(),
            shardSettings, new TypedMessageExtractorWithShardResolver<_,_>(EntityRefs.entityRefExtractor, shardResolver))
    { ShardRegion = shardRegion; TypeName = name }
