...
module Dispatcher =
    ...
    let default_executor =
        { new BaseBuilder<Akka.Actor.DefaultExecutor.Field>() with
            member _.Run(state: string list) =
                objExpr "default-executor" 4 state |> Akka.Actor.DefaultExecutor.Field }
    ...
    type ForkJoinExecutorBuilder<'T when 'T :> IField>() =
        ...
        member this.parallelism_min value =
            this.ParallelismMin([], value) |> this.Run
        ...
        member this.parallelism_factor value =
            this.ParallelismFactor([], value) |> this.Run
        ...
        member this.parallelism_max value =
            this.ParallelismMax([], value) |> this.Run
        ...
        member this.task_peeking_mode value =
            this.TaskPeekingMode([], value) |> this.Run
    ...
    let fork_join_executor =
        { new ForkJoinExecutorBuilder<Akka.Actor.ForkJoinExecutor.Field>() with
            member _.Run(state: string list) =
                objExpr "fork-join-executor" 4 state |> Akka.Actor.ForkJoinExecutor.Field }

    /// This will be used if you have set "executor = "thread-pool-executor""
    let thread_pool_executor =
        { new BaseBuilder<Akka.Actor.ThreadPoolExecutor.Field>() with
            member _.Run(state: string list) =
                objExpr "thread-pool-executor" 4 state |> Akka.Actor.ThreadPoolExecutor.Field }
    ...
    type DedicatedThreadPoolBuilder<'T when 'T :> IField>() =
        ...
        /// Number of threads
        member this.thread_count value = this.ThreadCount([], value) |> this.Run
        ...
        member inline this.deadlock_timeout(value: 'Duration) =
            this.DeadlockTimeout([], value) |> this.Run

        [<CustomOperation("threadtype"); EditorBrowsable(EditorBrowsableState.Never)>]
        member _.Threadtype(state: string list, x: Hocon.ThreadType) = field "threadtype" x.Text :: state

        member this.threadtype value = this.Threadtype([], value) |> this.Run
    ...
    let dedicated_thread_pool =
        { new DedicatedThreadPoolBuilder<Akka.Actor.DedicatedThreadPool.Field>() with
            member _.Run(state: string list) =
                objExpr "dedicated-thread-pool" 4 state |> Akka.Actor.DedicatedThreadPool.Field }
    ...
    type DispatcherBuilder<'T when 'T :> IField>(name: string, mkField: string -> 'T, path: string, indent: int) =
        ...
        member this.attempt_teamwork value =
            this.AttemptTeamwork([], value) |> this.Run
        ...
        member _.Type'(state: string list, x: Hocon.Dispatcher) = field "type" x.Text :: state

        [<EditorBrowsable(EditorBrowsableState.Never)>]
        member _.Type'(state: string list, x: string) = quotedField "type" x :: state
        ...
        ///
        /// PinnedDispatcher must be used together with executor=fork-join-executor.
        member this.type'(value: Hocon.Dispatcher) = this.Type'([], value) |> this.Run
        /// Must be one of the following
        /// Dispatcher, PinnedDispatcher, or a FQCN to a class inheriting
        /// MessageDispatcherConfigurator with a public constructor with
        /// both com.typesafe.config.Config parameter and
        ...
        ///
        /// PinnedDispatcher must be used together with executor=fork-join-executor.
        member this.type'(value: string) = this.Type'([], value) |> this.Run
        ...
        member _.Executor(state: string list, x: Hocon.Executor) = quotedField "executor" x.Text :: state

        [<EditorBrowsable(EditorBrowsableState.Never)>]
        member _.Executor(state: string list, x: string) = quotedField "executor" x :: state
        ...
        ///
        /// - A FQCN of a class extending ExecutorServiceConfigurator
        member this.executor(value: Hocon.Executor) = this.Executor([], value) |> this.Run
        /// Which kind of ExecutorService to use for this dispatcher
        ///
        /// Valid options:
        /// - "default-executor" requires a "default-executor" section
        ...
        ///
        /// - A FQCN of a class extending ExecutorServiceConfigurator
        member this.executor(value: string) = this.Executor([], value) |> this.Run
        ...
        member this.mailbox_requirement value =
            this.MailboxRequirement([], value) |> this.Run
        ...
        ///
        /// Set to 1 for as fair as possible.
        member this.throughput value = this.Throughput([], value) |> this.Run
        ...
        member inline this.throughput_deadline_time(value: 'Duration) =
            this.ThroughputDeadlineTime([], value) |> this.Run
        ...
        member _.current_context_executor =
            { new BaseBuilder<'T>() with
                member _.Run(state: string list) =
                    pathObjExpr "current-context-executor" state }
        ...
        member _.thread_pool_executor =
            { new BaseBuilder<'T>() with
                member _.Run(state: string list) =
                    pathObjExpr "thread-pool-executor" state }

        member _.dedicated_thread_pool =
            { new DedicatedThreadPoolBuilder<'T>() with
                member _.Run(state: string list) =
                    pathObjExpr "dedicated-thread-pool" state }
    ...
    let custom_dispatcher (name: string) =
        ...
        { new DispatcherBuilder<Akka.Actor.Dispatcher.Field>(name, Akka.Actor.Dispatcher.Field, name, indent) with
            member _.Run(state: string list) =
                objExpr name indent state |> Akka.Actor.Dispatcher.Field }
    ...
    let default_blocking_io_dispatcher =
        custom_dispatcher "default-blocking-io-dispatcher"
