...
module PubSub =
    ...
    type PubSubBuilder<'T when 'T :> IField>() =
        ...
        member _.RoutingLogic(state: string list, x: Hocon.Router.Logic) = field "routing-logic" (x.Text) :: state

        /// The routing logic to use for 'Send'
        /// Possible values: random, round-robin, broadcast
        member this.routing_logic value =
            this.RoutingLogic([], value) |> this.Run
        ...
        member inline this.gossip_interval(value: 'Duration) =
            this.GossipInterval([], value) |> this.Run
        ...
        member inline this.removed_time_to_live(value: 'Duration) =
            this.RemovedTimeToLive([], value) |> this.Run
        ...
        member this.max_delta_elements value =
            this.MaxDeltaElements([], value) |> this.Run
    ...
    let pub_sub =
        { new PubSubBuilder<Akka.Cluster.Field>() with
            member _.Run(state: string list) =
                objExpr "pub-sub" 3 state |> Akka.Cluster.Field }
