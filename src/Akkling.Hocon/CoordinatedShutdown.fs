...
module CoordinatedShutdown =
    ...
    type PhaseBuilder<'T when 'T :> IField>(name: string) =
        ...
        member _.DependsOn(state: string list, values: string list) = listField "depends-on" values :: state
        ...
        member this.depends_on(value: string list) = this.DependsOn([], value) |> this.Run
        member this.depends_on(value: PhaseBuilder<_> list) = this.DependsOn([], value) |> this.Run
    ...
    let private createPhaseBuilder name =
        { new PhaseBuilder<Akka.CoordinatedShutdown.Phases.Field>(name) with
            member _.Run(state: string list) =
                objExpr name 4 state |> Akka.CoordinatedShutdown.Phases.Field }
    ...
    let cluster_sharding_shutdown_region =
        createPhaseBuilder "cluster-sharding-shutdown-region"
    ...
    let before_actor_system_terminate =
        createPhaseBuilder "before-actor-system-terminate"
    ...
    type PhasesBuilder<'T when 'T :> IField>(mkField: string -> 'T, path: string, indent: int) =
        ...
        /// Graceful shutdown of the Cluster Sharding regions.
        member this.cluster_sharding_shutdown_region =
            this.createPhaseBuilder "cluster-sharding-shutdown-region"
        ...
        /// Phase for custom application tasks that are to be run
        /// after cluster shutdown and before ActorSystem termination.
        member this.before_actor_system_terminate =
            this.createPhaseBuilder "before-actor-system-terminate"
    ...
    let phases =
        ...
        { new PhasesBuilder<Akka.CoordinatedShutdown.Field>(Akka.CoordinatedShutdown.Field, path, indent) with
            member _.Run(state: string list) =
                objExpr path indent state |> Akka.CoordinatedShutdown.Field }
    ...
    type CoordinatedShutdownBuilder<'T when 'T :> IField>() =
        ...
        member inline this.default_phase_timeout(value: 'Duration) =
            this.DefaultPhaseTimeout([], value) |> this.Run
        ...
        member this.terminate_actor_system value =
            this.TerminateActorSystem([], value) |> this.Run
        ...
        member _.ExitClr(state: string list, value: bool) = switchField "exit-clr" value :: state

        /// Exit the CLR(Environment.Exit(0)) in the last phase actor-system-terminate
        /// if this is set to 'on'. It is done after termination of the
        /// ActorSystem if terminate-actor-system=on, otherwise it is done
        /// immediately when the last phase is reached.
        member this.exit_clr value = this.ExitClr([], value) |> this.Run
        ...
        member _.RunByClrShutdownHook(state: string list, value: bool) = switchField "exit-clr" value :: state
        ...
        member this.run_by_clr_shutdown_hook value =
            this.RunByClrShutdownHook([], value) |> this.Run
        ...
        member this.run_by_actor_system_terminate value =
            this.RunByActorSystemTerminate([], value) |> this.Run
    ...
    let coordinated_shutdown =
        { new CoordinatedShutdownBuilder<Akka.Field>() with
            member _.Run(state: string list) =
                objExpr "coordinated-shutdown" 2 state |> Akka.Field }
