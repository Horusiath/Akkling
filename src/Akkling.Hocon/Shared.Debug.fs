...
module Debug =
    ...
    type DebugBuilder<'T when 'T :> IField>() =
        ...
        member _.Combine(state1: State<Debug.Target>, state2: State<Debug.Target>) : State<Debug.Target> =
            ...
            | None -> failwithf "Debug fields mismatch found was given:%s%A" System.Environment.NewLine fields
        ...
        member _.AutoReceive(state: State<Debug.Target>, value: bool) =
            switchField "autoreceive" value |> State.addWith state Debug.Target.Actor

        /// Enable DEBUG logging of all AutoReceiveMessages (Kill, PoisonPill et.c.)
        member this.autoreceive value =
            this.AutoReceive(State.create Debug.Target.Actor [], value) |> this.Run
        ...
        member _.EventStream(state: State<Debug.Target>, value: bool) =
            switchField "event-stream" value |> State.addWith state Debug.Target.Actor

        /// Enable DEBUG logging of subscription changes on the eventStream
        member this.event_stream value =
            this.EventStream(State.create Debug.Target.Actor [], value) |> this.Run
        ...
        member _.Fsm(state: State<Debug.Target>, value: bool) =
            switchField "fsm" value |> State.addWith state Debug.Target.Actor

        /// Enable DEBUG logging of all LoggingFSMs for events, transitions and timers
        member this.fsm value =
            this.Fsm(State.create Debug.Target.Actor [], value) |> this.Run
        ...
        member _.Lifecycle(state: State<Debug.Target>, value: bool) =
            switchField "lifecycle" value |> State.addWith state Debug.Target.Actor

        /// Enable DEBUG logging of actor lifecycle changes
        member this.lifecycle value =
            this.Lifecycle(State.create Debug.Target.Actor [], value) |> this.Run
        ...
        member _.Receive(state: State<Debug.Target>, value: bool) =
            switchField "receive" value |> State.addWith state Debug.Target.Actor

        /// Enable function of Actor.loggable(), which is to log any received message
        /// at DEBUG level, see the “Testing Actor Systems” section of the Akka
        /// Documentation at http://akka.io/docs
        member this.receive value =
            this.Receive(State.create Debug.Target.Actor [], value) |> this.Run
        ...
        member _.Unhandled(state: State<Debug.Target>, value: bool) =
            switchField "unhandled" value |> State.addWith state Debug.Target.Actor

        /// Enable DEBUG logging of unhandled messages
        member this.unhandled value =
            this.Unhandled(State.create Debug.Target.Actor [], value) |> this.Run
    ...
    let debug =
        { new DebugBuilder<Akka.Shared.Debug.Field>() with
            member _.Run(state: State<Debug.Target>) : Akka.Shared.Field<Akka.Shared.Debug.Field> =
                fun indent -> objExpr "debug" indent state.Fields |> Akka.Shared.Debug.Field }
