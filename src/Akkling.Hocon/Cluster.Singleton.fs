...
module ClusterSingleton =
    ...
    type SingletonProxyBuilder<'T when 'T :> IField>() =
        ...
        member _.SingletonName(state: string list, x: string) = quotedField "singleton-name" x :: state

        /// The actor name of the singleton actor that is started by the ClusterSingletonManager
        member this.singleton_name value =
            this.SingletonName([], value) |> this.Run
        ...
        member inline this.singleton_identification_interval(value: 'Duration) =
            this.SingletonIdentificationInterval([], value) |> this.Run
        ...
        member _.BufferSize(state: string list, value: int) =
            ...
            if value > 10000 then
                failwithf "Field %s must have a value between 0 and 10000. You provided: %i" fieldName value
            else
                positiveField fieldName value :: state
    ...
    let singleton_proxy =
        { new SingletonProxyBuilder<Akka.Cluster.Field>() with
            member _.Run(state: string list) =
                objExpr "singleton-proxy" 3 state |> Akka.Cluster.Field }
    ...
    type SingletonBuilder<'T when 'T :> IField>() =
        ...
        member inline this.hand_over_retry_interval(value: 'Duration) =
            this.HandOverRetryInterval([], value) |> this.Run
        ...
        member this.min_number_of_hand_over_retries value =
            this.MinNumberOfHandOverRetries([], value) |> this.Run

    let singleton =
        { new SingletonBuilder<Akka.Cluster.Field>() with
            member _.Run(state: string list) =
                objExpr "singleton" 3 state |> Akka.Cluster.Field }
