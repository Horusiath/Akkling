...
module Udp =
    ...
    type UdpBuilder<'T when 'T :> IField>(mkField: string -> 'T, path: string) =
        ...
        member _.Yield(x: Akka.IO.DisabledBufferPool.Field) =
            State.create Udp.Target.IO [ x.ToString() ]
        ...
        member _.Yield(x: Akka.Remote.Batching.Field) =
            State.create Udp.Target.Remote [ x.ToString() ]
        ...
        member _.Combine(state1: State<Udp.Target>, state2: State<Udp.Target>) : State<Udp.Target> =
            ...
            | None -> failwithf "Udp fields mismatch found was given:%s%A" System.Environment.NewLine fields
        ...
        member _.BufferPool(state: State<Udp.Target>, value: string) =
            quotedField "buffer-pool" value |> State.addWith state Udp.Target.IO
        ...
        member this.buffer_pool value =
            this.BufferPool(State.create Udp.Target.IO [], value) |> this.Run
        ...
        member inline this.nr_of_socket_async_event_args value =
            this.NrOfSocketAsyncEventArgs(State.create Udp.Target.IO [], value) |> this.Run
        ...
        member inline _.MaxChannels(state: State<Udp.Target>, value) =
            positiveField "max-channels" value |> State.addWith state Udp.Target.IO
        ...
        member this.max_channels(value: Hocon.Unlimited) =
            this.MaxChannels(State.create Udp.Target.IO [], value) |> this.Run
        ...
        member inline _.SelectTimeout(state: State<Udp.Target>, value: 'Duration) =
            durationField "select-timeout" value |> State.addWith state Udp.Target.IO

        [<EditorBrowsable(EditorBrowsableState.Never)>]
        member _.SelectTimeout(state: State<Udp.Target>, value: Hocon.Infinite) =
            field "select-timeout" (value.ToString()) |> State.addWith state Udp.Target.IO
        ...
        member this.select_timeout(value: Hocon.Infinite) =
            this.SelectTimeout(State.create Udp.Target.IO [], value) |> this.Run
        ...
        member inline _.ReceiveThroughput(state: State<Udp.Target>, value) =
            positiveField "receive-throughput" value |> State.addWith state Udp.Target.IO

        /// The maximum number of datagrams that are read in one go,
        /// higher numbers decrease latency, lower numbers increase fairness on
        /// the worker-dispatcher
        member inline this.receive_throughput value =
            this.ReceiveThroughput(State.create Udp.Target.IO [], value) |> this.Run
        ...
        member inline _.DirectBufferSize(state: State<Udp.Target>, value) =
            positiveField "direct-buffer-size" value |> State.addWith state Udp.Target.IO

        /// The number of bytes per direct buffer in the pool used to read or write
        /// network data from the kernel.
        member inline this.direct_buffer_size value =
            this.DirectBufferSize(State.create Udp.Target.IO [], value) |> this.Run
        ...
        member inline this.direct_buffer_pool_limit value =
            this.DirectBufferPoolLimit(State.create Udp.Target.IO [], value) |> this.Run
        ...
        member this.received_message_size_limit(value: Hocon.Unlimited) =
            this.ReceivedMessageSizeLimit(State.create Udp.Target.IO [], value) |> this.Run
        ...
        member _.TraceLogging(state: State<Udp.Target>, value: bool) =
            switchField "trace-logging" value |> State.addWith state Udp.Target.IO

        /// Enable fine grained logging of what goes on inside the implementation.
        /// Be aware that this may log more than once per message sent to the actors
        /// of the tcp implementation.
        member this.trace_logging value =
            this.TraceLogging(State.create Udp.Target.IO [], value) |> this.Run
        ...
        member _.SelectorDispatcher(state: State<Udp.Target>, value: string) =
            quotedField "selector-dispatcher" value |> State.addWith state Udp.Target.IO

        /// Fully qualified config path which holds the dispatcher configuration
        /// to be used for running the select() calls in the selectors
        member this.selector_dispatcher value =
            this.SelectorDispatcher(State.create Udp.Target.IO [], value) |> this.Run
        ...
        member _.WorkerDispatcher(state: State<Udp.Target>, value: string) =
            quotedField "worker-dispatcher" value |> State.addWith state Udp.Target.IO

        /// Fully qualified config path which holds the dispatcher configuration
        /// for the read/write worker actors
        member this.worker_dispatcher value =
            this.WorkerDispatcher(State.create Udp.Target.IO [], value) |> this.Run
        ...
        member _.ManagementDispatcher(state: State<Udp.Target>, value: string) =
            quotedField "management-dispatcher" value |> State.addWith state Udp.Target.IO

        /// Fully qualified config path which holds the dispatcher configuration
        /// for the selector management actors
        member this.management_dispatcher value =
            this.ManagementDispatcher(State.create Udp.Target.IO [], value) |> this.Run
        ...
        member _.TransportClass(state: State<Udp.Target>, x: string) =
            quotedField "transport-class" x |> State.addWith state Udp.Target.Remote
        ...
        member this.transport_class value =
            this.TransportClass(State.create Udp.Target.Remote [], value) |> this.Run
        ...
        member _.AppliedAdapters(state: State<Udp.Target>, x: string list) =
            quotedListField "applied-adapters" x |> State.addWith state Udp.Target.Remote
        ...
        member this.applied_adapters value =
            this.AppliedAdapters(State.create Udp.Target.Remote [], value) |> this.Run
        ...
        member _.ByteOrder(state: State<Udp.Target>, x: Hocon.ByteOrder) =
            quotedField "byte-order" (x.Text) |> State.addWith state Udp.Target.Remote

        /// Byte order used for network communication. Event thou DotNetty is big-endian
        /// by default, we need to switch it back to little endian in order to support
        /// backward compatibility with Helios.
        member this.byte_order value =
            this.ByteOrder(State.create Udp.Target.Remote [], value) |> this.Run
        ...
        member inline _.Port(state: State<Udp.Target>, x) =
            positiveField "port" x |> State.addWith state Udp.Target.Remote

        /// The default remote server port clients should connect to.
        /// Default is 2552 (AKKA), use 0 if you want a random available port
        /// This port needs to be unique for each actor system on the same machine.
        member inline this.port value =
            this.Port(State.create Udp.Target.Remote [], value) |> this.Run
        ...
        member inline _.PublicPort(state: State<Udp.Target>, x) =
            positiveField "public-port" x |> State.addWith state Udp.Target.Remote
        ...
        member inline this.public_port value =
            this.PublicPort(State.create Udp.Target.Remote [], value) |> this.Run
        ...
        member _.Hostname(state: State<Udp.Target>, x: string) =
            field "hostname" x |> State.addWith state Udp.Target.Remote

        /// The hostname or ip to bind the remoting to,
        /// InetAddress.getLocalHost.getHostAddress is used if empty
        member this.hostname value =
            this.Hostname(State.create Udp.Target.Remote [], value) |> this.Run
        ...
        member _.PublicHostname(state: State<Udp.Target>, x: string) =
            field "public-hostname" x |> State.addWith state Udp.Target.Remote

        /// If this value is set, this becomes the public address for the actor system on this
        /// transport, which might be different than the physical ip address (hostname)
        /// this is designed to make it easy to support private / public addressing schemes
        member this.public_hostname value =
            this.PublicHostname(State.create Udp.Target.Remote [], value) |> this.Run
        ...
        member _.DnsUseIpv6(state: State<Udp.Target>, value: bool) =
            boolField "dns-use-ipv6" value |> State.addWith state Udp.Target.Remote

        /// If set to true, we will use IPV6 addresses upon DNS resolution for host names.
        /// Otherwise, we will use IPV4.
        member this.dns_use_ipv6 value =
            this.DnsUseIpv6(State.create Udp.Target.Remote [], value) |> this.Run
        ...
        member _.EnforceIpFamily(state: State<Udp.Target>, value: bool) =
            boolField "enforce-ip-family" value |> State.addWith state Udp.Target.Remote
        ...
        member this.enforce_ip_family value =
            this.EnforceIpFamily(State.create Udp.Target.Remote [], value) |> this.Run
        ...
        member _.EnableSsl(state: State<Udp.Target>, value: bool) =
            boolField "enable-ssl" value |> State.addWith state Udp.Target.Remote

        /// Enables SSL support on this transport
        member this.enable_ssl value =
            this.EnableSsl(State.create Udp.Target.Remote [], value) |> this.Run
        ...
        member inline this.connection_timeout(value: 'Duration) =
            this.ConnectionTimeout(State.create Udp.Target.Remote [], value) |> this.Run
        ...
        member _.UseDispatcherForIo(state: State<Udp.Target>, x: string) =
            quotedField "use-dispatcher-for-io" x |> State.addWith state Udp.Target.Remote
        ...
        member this.use_dispatcher_for_io value =
            this.UseDispatcherForIo(State.create Udp.Target.Remote [], value) |> this.Run
        ...
        member inline _.SendBufferSize(state: State<Udp.Target>, value) =
            byteField "send-buffer-size" value |> State.addWith state Udp.Target.Remote

        /// Sets the send buffer size of the Sockets,
        /// set to 0b for platform default
        member inline this.send_buffer_size value =
            this.SendBufferSize(State.create Udp.Target.Remote [], value) |> this.Run
        ...
        member inline _.ReceiveBufferSize(state: State<Udp.Target>, value) =
            byteField "receive-buffer-size" value |> State.addWith state Udp.Target.Remote

        /// Sets the receive buffer size of the Sockets,
        /// set to 0b for platform default
        member inline this.receive_buffer_size value =
            this.ReceiveBufferSize(State.create Udp.Target.Remote [], value) |> this.Run
        ...
        member inline _.MaximumFrameSize(state: State<Udp.Target>, value) =
            byteField "maximum-frame-size" value |> State.addWith state Udp.Target.Remote
        ...
        member inline this.maximum_frame_size value =
            this.MaximumFrameSize(State.create Udp.Target.Remote [], value) |> this.Run
        ...
        member inline _.Backlog(state: State<Udp.Target>, value) =
            positiveField "backlog" value |> State.addWith state Udp.Target.Remote

        /// Sets the size of the connection backlog
        member inline this.backlog value =
            this.Backlog(State.create Udp.Target.Remote [], value) |> this.Run
        ...
        member _.TransportProtocol(state: State<Udp.Target>, x: string) =
            field "transport-protocol" x |> State.addWith state Udp.Target.Remote

        /// configured manually to point to the same dispatcher if needed.
        member this.transport_protocol value =
            this.TransportProtocol(State.create Udp.Target.Remote [], value) |> this.Run
        ...
        member _.direct_buffer_pool =
            { new DirectBufferPoolBuilder<'T>() with
                member _.Run(state: string list) =
                    pathObjExpr 0 "direct-buffer-pool" state }

        member _.client_socket_worker_pool =
            { new WorkerPoolBuilder<'T>() with
                member _.Run(state: string list) =
                    pathObjExpr 0 "client-socket-worker-pool" state }

        member _.server_socket_worker_pool =
            { new WorkerPoolBuilder<'T>() with
                member _.Run(state: string list) =
                    pathObjExpr 0 "server-socket-worker-pool" state }
    ...
    let udp =
        ...
        { new UdpBuilder<Akka.Shared.Udp.Field>(Akka.Shared.Udp.Field, path) with
            member _.Run(state: State<Udp.Target>) : Akka.Shared.Field<Akka.Shared.Udp.Field> =
                fun indent -> objExpr path indent state.Fields |> Akka.Shared.Udp.Field }
