...
module Actor =
    ...
    type ActorBuilder<'T when 'T :> IField>() =
        ...
        /// Default timeout for IActorRef.Ask.
        member inline this.ask_timeout(value: 'Duration) = this.AskTimeout([], value) |> this.Run
        /// Default timeout for IActorRef.Ask.
        member this.ask_timeout(value: Hocon.Infinite) = this.AskTimeout([], value) |> this.Run
        ...
        member inline this.creation_timeout(value: 'Duration) =
            this.CreationTimeout([], value) |> this.Run
        ...
        member _.GuardianSupervisorStrategy(state: string list, x: Type) =
            if not (typeof<SupervisorStrategyConfigurator>.IsAssignableFrom x) then
                failwithf
                    "guardian_supervisor_strategy class must inherit from '%s'"
                    (fqcn typeof<SupervisorStrategyConfigurator>)
        ...
        member this.guardian_supervisor_strategy value =
            this.GuardianSupervisorStrategy([], value) |> this.Run
        ...
        member this.reaper_interval value =
            this.ReaperInterval([], value) |> this.Run
        ...
        member this.serialize_messages value =
            this.SerializeMessages([], value) |> this.Run
        ...
        member this.serialize_creators value =
            this.SerializeCreators([], value) |> this.Run
        ...
        member _.SerializationBindings(state: string list, xs: (System.Type * string) list) =
            stringyObjExpr "serialization-bindings" 4 (xs |> List.map (fun (t, v) -> t.FullName, v))
            :: state
        ...
        member this.serialization_bindings(xs: (string * string) list) =
            this.SerializationBindings([], xs) |> this.Run
        ...
        member _.SerializationIdentifiers(state: string list, xs: (Type * int) list) =
            stringyObjExpr "serialization-identifiers" 4 (xs |> List.map (fun (t, v) -> fqcn t, string v))
            :: state
        ...
        member this.serialization_identifiers(xs: (Type * int) list) =
            this.SerializationIdentifiers([], xs) |> this.Run
        ...
        member inline this.unstarted_push_timeout(value: 'Duration) =
            this.UnstartedPushTimeout([], value) |> this.Run
        ...
        member this.backoff_remote_dispatcher =
            this.custom_dispatcher "backoff-remote-dispatcher"

        member this.calling_thread_dispatcher =
            this.custom_dispatcher "calling-thread-dispatcher"
        ...
        member this.default_blocking_io_dispatcher =
            this.custom_dispatcher "default-blocking-io-dispatcher"

        member this.default_fork_join_dispatcher =
            this.custom_dispatcher "default-fork-join-dispatcher"

        member this.default_plugin_dispatcher =
            this.custom_dispatcher "default-plugin-dispatcher"

        member this.default_remote_dispatcher =
            this.custom_dispatcher "default-remote-dispatcher"

        member this.default_replay_dispatcher =
            this.custom_dispatcher "default-replay-dispatcher"

        member this.default_stream_dispatcher =
            this.custom_dispatcher "default-stream-dispatcher"
        ...
        member _.serialization_settings =
            { new BaseBuilder<Akka.Field>() with
                member _.Run(state: string list) =
                    pathObjExpr "serialization-settings" state }
    ...
    let actor =
        { new ActorBuilder<Akka.Field>() with
            member _.Run(state: string list) = objExpr "actor" 2 state |> Akka.Field }
