...
module Dns =
    ...
    type INetAddressBuilder<'T when 'T :> IField>() =
        ...
        member this.provider_object value =
            this.ProviderObject([], value) |> this.Run
        ...
        member inline this.positive_ttl(value: 'Duration) = this.PositiveTtl([], value) |> this.Run
        ...
        member inline this.negative_ttl(value: 'Duration) = this.NegativeTtl([], value) |> this.Run
        ...
        member inline this.cache_cleanup_interval(value: 'Duration) =
            this.CacheCleanupInterval([], value) |> this.Run

        [<CustomOperation("use_ipv6"); EditorBrowsable(EditorBrowsableState.Never)>]
        member _.UseIpv6(state: string list, value: bool) = boolField "use-ipv6" value :: state

        member this.use_ipv6 value = this.UseIpv6([], value) |> this.Run

    let inet_address =
        { new INetAddressBuilder<Akka.IO.INetAddress.Field>() with
            member _.Run(state: string list) =
                objExpr "inet-address" 4 state |> Akka.IO.INetAddress.Field }
    ...
    type DnsBuilder<'T when 'T :> IField>(mkField: string -> 'T, path: string, indent: int) =
        ...
        member _.Resolver(state: string list, value: string) = quotedField "resolver" value :: state

        /// Name of the subconfig at path akka.io.dns
        member this.resolver value = this.Resolver([], value) |> this.Run
    ...
    let dns =
        ...
        { new DnsBuilder<Akka.IO.Field>(Akka.IO.Field, path, indent) with
            member _.Run(state: string list) =
                objExpr path indent state |> Akka.IO.Field }
