...
let allwaysHandled _ n =
    printfn "always handled: %s" n |> ignored

let onlyWhenHanled _ n =
    printfn "combined behavior: %s" n |> ignored

let orRef =
    spawn system "or-actor" <| props (actorOf2 (helloBehavior <|> allwaysHandled))

let andRef =
    spawn system "and-actor" <| props (actorOf2 (helloBehavior <&> onlyWhenHanled))
